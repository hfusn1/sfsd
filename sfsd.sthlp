{smcl}
{cmd:help sfsd}{right:also see:  {help sfsd postestimation}}
{hline}

{title:Title}

{p2colset 5 13 15 2}{...}
{p2col :{hi:sfsd} {hline 2}}Spatial Durbin stochastic frontier models with inefficiency spillovers fitting {help sfsd##Galli2023:{bind:Galli (2023)}} {p_end}
{p2colreset}{...}
...
{p 8 17 2}
{cmd:sfsd} {depvar} [{indepvars}] [{cmd:,} {it:options}]

{pstd}
Version syntax

{p 8 17 2}
{cmd:sfsd}{cmd:,} {opt ver:sion}

{pstd}
Replay syntax

{p 8 17 2}
{cmd:sfsd} [{cmd:,} {cmdab:l:evel(}{help level##remarks:{it:#}}{cmd:)}]
...
{synopt :{cmd:vhet(}{it:varlist} [{cmd:,} {opt nocons:tant}]{cmd:)}}
specify explanatory
variables for the idiosyncratic error variance function;
use {opt noconstant} to suppress constant term. {p_end}
...
{synopt :{opt genwvars}}generate the spatial Durbin and spatial lag terms and spatial inefficiency components{p_end}
...
{p2colreset}{...}
{p 4 6 2}
See {help sfsd postestimation} for
...
{pstd}
{opt sfsd} fits spatial Durbin stochastic production or cost frontier models
following the methodology of 
{help sfsd##Galli2023:{bind:Galli (2023)}}. See 
{help sfsd##Galli2023:{bind:Galli (2023)}} for a detailed
...
specifies that the technical inefficiency component is heteroscedastic,
...
{phang}{cmd:vhet(}{it:varlist}[{cmd:,} {cmd:noconstant}]{cmd:)}
specifies that the idiosyncratic error component is heteroscedastic,
with the variance expressed as a function of the covariates defined in
{it:varlist}. Specifying {cmd:noconstant} suppresses 
the constant in this function.
...
If one weight matrix is specified, it assumes time-invariant weight matrix. 
...
If only one matrix is stored in the specified array, the time-invariant weight matrix is assumed.  
...
{phang}
{cmd:version} displays the version of {cmd:sfsd} installed on Stata and the
...
specifies linear constraints for the estimated model.
    ...
    {title:Examples}
        ...
        {title:SD-SF model with time-invariant spatial weight matrix}
    
    {pstd}
    Setup{p_end}
    {phang2}{bf:. {stata "mata mata matuse w_ex1,replace"}}{p_end}
    {phang2}{bf:. {stata "use sfsd_ex1.dta"}}{p_end}
    
    {pstd}
    Stochastic Durbin production model {p_end}
    {phang2}{bf:. {stata "sfsd y x, id(id) time(t) noconstant wymat(wm,mata) wxmat(wm,mata) wumat(wm,mata) mu(z) wxvars(x) wmuvars(z) "}}{p_end}
    {pstd}
    In the above case, Wy=Wx=Wu, it is equilvalent to use wmat() option as follows. {p_end}
    {phang2}{bf:. {stata "sfsd y x, id(id) time(t) noconstant wmat(wm,mata) mu(z) wxvars(x) wmuvars(z) "}}{p_end}
    
    
        {title:SD-SF model with time-invariant spatial weight matrix and unblanced panel}
    ...
    {pstd}
    Stochastic Durbin production model {p_end}
    {phang2}{bf:. {stata "sfsd y x,id(id) time(t) noconstant wmat(wm,mata) mu(z) wxvars(x) wmuvars(z) delmissing"}}{p_end}
    ...
    {phang2}{bf:. {stata "use sfsd_ex2.dta"}}{p_end}
    {phang2}{bf:. {stata "local w w1 w2 w3 w4 w5 w6 w7 w8 w9 w10"}}{p_end}
    
    {pstd}
    Stochastic cost model. {p_end}
    {phang2}{bf:. {stata "sfsd y x, id(id) time(t) cost noconstant wmat(`w',mata) mu(z) wxvars(x) wmuvars(z) mex(x) meu(z) "}}{p_end}
...
{pstd}
{cmd:sfsd} stores the following in {cmd:e()}:
...
{pstd}
{cmd:sfsd} is not an official Stata command.  It is a third-party command
programmed as a free contribution
to the research society.  By choosing to download, install, and use the
{cmd:sfsd} package, users assume all the liability for any
{cmd:sfsd}-package-related risk.  If you encounter any problems with the
{cmd:sfsd} package, or if you have comments, suggestions, or questions, please
...
{p 7 14 2}{helpb sfsd_postestimation} {helpb sfpanel}, {helpb sfkk},  {helpb nwxtregress} (if installed)
